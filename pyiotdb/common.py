...
from pyiotdb import exceptions
...
class DBAPICursor(with_metaclass(abc.ABCMeta, object)):
    ...
    def fetchone(self):
        ...
        if self._state == self._STATE_NONE:
            raise exceptions.ProgrammingError("No query yet")
...
class ParamEscaper(object):
    ...
    def escape_args(self, parameters):
        if isinstance(parameters, dict):
            ...
            raise exceptions.ProgrammingError("Unsupported param format: {}".format(parameters))
    ...
    def escape_item(self, item):
        if item is None:
            ...
            raise exceptions.ProgrammingError("Unsupported object {}".format(item))
